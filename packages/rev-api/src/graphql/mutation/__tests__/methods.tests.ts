...
describe('getModelMethodMutations()', () => {
    ...
    it.skip('should register all Post methods', () => {
    ...
    it.skip('TODO: Check over the other test cases for mutator methods...', () => {
        expect(Object.keys(config)).to.have.length(3);
    });

    // it.skip('methods with modelData=true should have a required model parameter', () => {
    //     expect(config['Post_postMethod2'].args['model'].type.toString().endsWith('!')).to.be.true;
    // });

    // it.skip('methods with modelData=false should not have a model parameter', () => {
    //     expect(config['Post_postMethod1'].args['model']).not.to.exist;
    // });

    // it.skip('method "model" args should be of the Model_input type.', () => {
    //     expect(config['Post_postMethod2'].args['model'].type.toString()).to.contain('Post_input');
    // });

    // it.skip('all methods should have a return type of JSON', () => {
    //     expect(config['Post_postMethod1'].type.toString()).to.equal('JSON');
    //     expect(config['Post_postMethod2'].type.toString()).to.equal('JSON');
    // });

    // it.skip('methods that do not require args should not register args', () => {
    //     expect(Object.keys(config['Post_postMethod3'].args)).to.have.length(0);
    // });

    // it.skip('methods that only require the "model" arg should register as expected', () => {
    //     expect(Object.keys(config['Post_postMethod2'].args)).to.have.length(1);
    //     expect(config['Post_postMethod2'].args['model']).to.exist;
    // });

    // it.skip('methods that only require custom args should register as expected', () => {
    ...
    // it.skip('methods should define a resolve function', () => {
