...
export let VALIDATION_MESSAGES = {
    ...
    invalid_record_class: (label: string) => `Invalid record class for field '${label}'`,
    missing_record_primary_key: (label: string) => `The record set for field '${label}' does not have a primary key value`,
    invalid_record_list_data: (label: string) => `${label} must be a list of records`,
    invalid_record_list_class: (label: string) => `One or more records in ${label} has the wrong class.`
};
