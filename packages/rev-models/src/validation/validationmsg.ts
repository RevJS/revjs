...
export let VALIDATION_MESSAGES = {
    ...
    invalid_model_class: (label: string) => `Invalid object class for field '${label}'`,
    missing_model_primary_key: (label: string) => `The object set for field '${label}' does not have a primary key value`,
    invalid_model_list_data: (label: string) => `${label} must be a list of objects`,
    invalid_model_list_class: (label: string) => `One or more objects in ${label} has the wrong class.`
};
